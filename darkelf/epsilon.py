...
def load_epsilon_grid(self, datadir, filename):
    ...
    # Reshapes to array in omega, k
    eps1df = data.pivot(index="omega", columns="k", values="eps1")
    eps1df = eps1df.interpolate()
    eps2df = data.pivot(index="omega", columns="k", values="eps2")
    eps2df = eps2df.interpolate()
    ...
    eps1arr = np.array(eps1df)
    eps2arr = np.array(eps2df)

    self._eps1_interp = RegularGridInterpolator(
        (omall, kall),
        eps1arr,
        fill_value=1.0,
        bounds_error=False,
    )  # set eps_1 to 1 when going outside the grid

    self._eps2_interp = RegularGridInterpolator(
        (omall, kall),
        eps2arr,
        fill_value=0.0,
        bounds_error=False,
    )  # set eps_2 to zero when going outside the grid
...
def load_Zion(self,datadir):
    ...
    if( not os.path.exists(Zion_path)):
        ...
        with open(Zion_path, encoding="latin1") as f:
            citation = f.readline().strip()
            print("Zion(k) for Migdal calculation taken or calculated from: " + citation)

            Ziondat = np.loadtxt(f).T

        self.Zion_k = interp1d(
            Ziondat[0],
            Ziondat[1],
            fill_value=(Ziondat[1][0], Ziondat[1][-1]),
            bounds_error=False
        )
